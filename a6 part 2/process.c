...
#include "userprog/syscall.h" //change3

static thread_func start_process NO_RETURN;
static bool load (const char *cmdline, void (**eip) (void), void **esp);
struct child * get_child(tid_t,struct thread *); //change3
...
process_execute (const char *file_name) 
{
  ...
  tid = thread_create (process_name, PRI_DEFAULT, start_process, fn_copy); //change3
  if (tid == TID_ERROR){
...
process_exit (void)
{
  ...
  ASSERT(list_empty(&thread_current()->files));
	//change4
...
static bool setup_stack (void **esp, char * cmdline); //change3
