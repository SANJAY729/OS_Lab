...
#include <threads/synch.h> //change3
...
struct child{
	tid_t tid;
	int status_exit;     //exit status
  bool load;			     //is load successful?
  bool is_parent_wait; //is parent waiting for this child?
...
struct thread
  {
    ...
    fixed_point recent_cpu;             //change2
		//change3
		struct list child_process_list;			//child processes list
    ...
    int total_open_files;               //total number of open files
    struct list open_files;             //list of file pointers and file descriptors
    int curr_open_fd;                   //fd of current process exec file
    int parent_open_fd;                 //fd of parent process exec file
		//change3
  };
